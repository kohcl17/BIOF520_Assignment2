...
g52 <- readRDS('./model/uromol_train_RF2_70pct_SHAP_genes.rds')
...
uromol.expr.train <- uromol.expr[idx, g52]
uromol.clin.train <- uromol.clin[idx, ]

knowles.expr <- knowles.expr[rownames(knowles.clin), g52]
```
...
uromol.train.coxph.df <- cbind(data.frame(uromol.expr.train.scaled),
...
uromol.train.risk <- as.matrix(uromol.train.coxph.df[, g52]) %*% uromol.train.cox_coefs
...
pr$curve # based on this, if I want higher recall of 0.625, I'd get precision of 0.282, and use ROR of -1 (approx) to classify them (row 31)
```

```{r}

uromol.train.coxph.df <- uromol.train.coxph.df %>%
  mutate(ROR_level = case_when(ROR_score > -1 ~ "High",
                               ROR_score <= -1 ~ "Low"))
```
...
evaluate.ROR <- function(expr.data, metadata, geneset, plot.title = "Training") {
  ...
  cox.df <- comb.df %>%
    mutate(ROR_score = risk[,1],
           ROR_level = case_when(ROR_score > -1 ~ "High",
                               ROR_score <= -1 ~ "Low"),
  ...
  return(list(km = ROR.kmplot, conf.mtx = ror.conf_mtx, cox.df = conf_mat_df))
}
```

```{r}

uromol.expr.test <- scale(uromol.expr[-idx, g52], center = train.means, scale = train.sd)
...
knowles.expr.scaled <- scale(knowles.expr[,g52], center = train.means, scale = train.sd)
