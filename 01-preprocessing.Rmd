...
uromol.clin[is.na(uromol.clin$RFS_time),] # have to use RFS/recurrence

uromol.clin.complete <- na.omit(uromol.clin, cols = c('RFS_time', 'Recurrence'))
```

```{r}

km <- with(uromol.clin.complete, Surv(RFS_time, Recurrence))
km_fit <- survfit(Surv(RFS_time, Recurrence) ~ BCG, data=uromol.clin.complete)
...
# ggsave("./plots/km_survival_BCG_stratified.pdf", p1, width = 6, height = 4)
```

```{r}

# retain only clinical parameters to use
uromol.clin.complete.int <- uromol.clin.complete[, c(4,5,7,8,9,12,13,14,15,16,17)]
uromol.clin.complete.int <- uromol.clin.complete.int %>%
  mutate(Tumor.size = factor(Tumor.size, 
         ...
         Recurrence = case_match(as.character(Recurrence),
                                 "0" ~ 'No',
                                 "1" ~ 'Yes'),
         recurrence_at_36mths = case_when(RFS_time <= 36 & Recurrence == "Yes" ~ "Yes",
                                         .default = "No"),
...
knowles.clin.complete <- knowles.clin[!is.na(knowles.clin$RFS_time),]
knowles.clin.complete.int <- knowles.clin.complete[, c(3,4,6,7)]
knowles.clin.complete.int <- knowles.clin.complete.int %>%
  mutate(
         Sex = factor(Sex, levels = c("M", "F")),
         Recurrence = case_match(as.character(Recurrence),
                                 "0" ~ 'No',
...
saveRDS(uromol.clin.complete.int, file = "./data_cleaned/clinical/uromol_clinical_data.rds")
saveRDS(knowles.clin.complete.int, file = "./data_cleaned/clinical/knowles_clinical_data.rds")
```
